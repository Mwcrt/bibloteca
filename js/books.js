document.addEventListener("DOMContentLoaded", function() {
    ...
    const booksPerPage = 10; // Número de libros por página
    let currentPage = 1;
...
document.querySelectorAll('.pagination button').forEach(button => {
    button.addEventListener('click', () => {
        const books = document.querySelectorAll('.book'); // Seleccionar todas las cartas
        books.forEach(book => {
            book.classList.remove('rotate'); // Reinicia la animación quitando la clase
            void book.offsetWidth; // Forzar reflujo para reiniciar la animación
            book.classList.add('rotate'); // Agrega la clase para activar la animación
        });
    });
});
